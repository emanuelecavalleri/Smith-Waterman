...
## Applying **Smith-Waterman** algorithm to strings
...
sw <- smith_waterman(X, Y, match, mismatch, gap)
sw
```
...
```{r}
cat("Mean execution time", round(end_time - start_time, 1) / 10, "second(s) on a(n)", get_cpu()$model_name, "having", round(get_ram()/1000000000, 2), "GB of RAM")
```
...
Printing **Smith-Waterman** algorithm's output permits you to bypass the problem associated with different type of objects as input. This function is the one suggested to locally align two nucleotide sequences in any format (i.e.: strings or DNAString objects) and visualize the output:

```{r}
print_smith_waterman(HNF4alpha[1], "AGGCCAAAGTCCT", match, mismatch, gap)
```
